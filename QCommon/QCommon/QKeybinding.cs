...
{
    public abstract class QKeybinding : UICustomControl
    {
        ...
        public void AddKeymappingsList()
        {
            foreach (KeyValuePair<string, SavedInputKey> keybinding in Keybindings)
            {
                AddKeymapping(keybinding.Key, keybinding.Value);
            }
        }
        ...
        private bool IsAlreadyBound(SavedInputKey target, InputKey inputKey, string category, out List<SavedInputKey> currentAssigned)
        {
            ...
            foreach (KeyValuePair<string, SavedInputKey> key in Keybindings)
            {
                if (inputKey == key.Value.value)
                {
                    currentAssigned.Add(key.Value);
                }
            }
